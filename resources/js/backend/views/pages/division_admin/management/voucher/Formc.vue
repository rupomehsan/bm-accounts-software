                                    ...
                                    <div class="admin_form form_1" v-for="(item, index) in support_voucher" :key=index>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <input type="file" :data-image="index" @change="resetError()"
                                                        :name="'support_voucher[' + index + '][image]'" class="form-control"
                                                        accept="image/*">
                                                </div>
                                                <div class="d-flex gap-1 mx-1">
                                                    <button type="button" class="btn btn-info btn-sm"
                                                        @click="ApprovedByAdmin(item.id)">{{item.approved_by_admin?'Approved':'Not approved'}}</button>
                                                    <button type="button" @click="ApprovalCancel(item.id)"
                                                        class="btn btn-info btn-sm">{{item.is_canceled?'Canceled':'Cancel'}}</button>
                                                </div>
                                                <i class="icon-minus" style="cursor: pointer;"
                                                    @click="deleteSupportVoucher(item.id ? { 'is_id': true, 'id': item.id } : { 'is_id': false, 'index': index })"></i>
                                            </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="cancel_comment" id="" cols="30" rows="5"></textarea>
                            </div>
                        </div>
            ...
            if (this.single_user) {
                ...
                if (this.single_user.support_voucher?.length) {
                    ...
                    this.single_user.support_voucher.forEach((data) => {
                        ...
                        dataList.approved_by_admin = data.approved_by_admin
