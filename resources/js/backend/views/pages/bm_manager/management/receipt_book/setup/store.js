    ...
    actions: {
        all: async function (url) {
            ...
            if (url) {
                ...
                response = await axios.get(
                    "account-receipt-books" + "?offset=" + this.page_limit
                );
            }
            this.all_data = response.data.data;
        },
        get_all_rejected_receipt_books: async function (url) {
            let response;
            // let page = `?page=${pageLimit}`;
            if (url) {
                response = await axios.get(
                    url + "&rejected=1&offset=" + this.page_limit
                );
            } else {
                response = await axios.get(
                    "account-receipt-books" +
                        "?rejected=1&offset=" +
                        this.page_limit
                );
            }
        ...
        update: async function (form, id) {
            ...
            let response = await axios.post(
                `account-receipt-books/${id}?_method=PATCH`,
                formData
            );
