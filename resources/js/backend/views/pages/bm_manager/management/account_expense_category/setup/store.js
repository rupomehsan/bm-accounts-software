...
export const account_expense_category_setup_store = defineStore(
    "account_expense_category_setup_store",
    {
        state: () => ({
            all_data: {},
            single_data: {},
            ...
            all: async function (url = null) {
                ...
                if (url) {
                    response = await axios.get(
                        url +
                            `&offset=${this.offset}&type=expense&search=${this.search}`
                    );
                } else {
                    response = await axios.get(
                        `account-categories?offset=${this.offset}&type=expense&search=${this.search}`
                    );
                }
