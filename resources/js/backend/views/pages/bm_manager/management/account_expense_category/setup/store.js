...
export const account_expense_category_setup_store = defineStore("account_expense_category_setup_store", {
    state: () => ({
        all_data: {},
        single_data: {},
        role_data: {},
    ...
    actions: {
        all: async function (url = null) {
            let response;
            // let page = `?page=${pageLimit}`;
            if (url) {
                response = await axios.get(url + `&type=expense&search=${this.search}`);
            } else {
                response = await axios.get(`account-categories?type=expense&search=${this.search}`);
            }
