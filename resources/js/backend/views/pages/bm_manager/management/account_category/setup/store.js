...
export const account_income_category_setup_store = defineStore("account_income_category_setup_store", {
    state: () => ({
        all_data: {},
        single_data: {},
        role_data: {},
    ...
    actions: {
        allIncome: async function (url = null) {
            ...
            if (url) {
                response = await axios.get(url + `&type=income&search=${this.search}`);
            } else {
                response = await axios.get(`account-categories?type=income&search=${this.search}`);
            }
